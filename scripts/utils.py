...
def add_lambda_permission_for_sqs(queue_name: str, function_name: str, aws_region: str, aws_account_id: str) -> Optional[dict]:
    """
    Grants permission for an SQS queue to invoke the specified Lambda function.

    This function allows an SQS queue to trigger the Lambda function by adding the necessary permission.

    Args:
        queue_name (str): The name of the SQS queue that will trigger the Lambda function.
        function_name (str): The name of the Lambda function that will be invoked by the SQS queue.
        aws_region (str): The AWS region where the resources are located.
        aws_account_id (str): The AWS account ID of the account that owns the Lambda function and SQS queue.
    ...
    try:
        response = lambda_client.add_permission(
            ...
            Action='lambda:InvokeFunction',
            SourceArn=f'arn:aws:sqs:{aws_region}:{aws_account_id}:{queue_name}'
        )
        logging.info(f"Permission granted to Lambda '{function_name}' to send messages to SQS.")
        return response
    except ClientError as e:
        if 'ResourceConflictException' in str(e):
            return {"already exists":True}
        logging.error(f"Error granting SQS permission to Lambda '{function_name}': {e.response['Error']['Message']}", exc_info=True)
        return None
